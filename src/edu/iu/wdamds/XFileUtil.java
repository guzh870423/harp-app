...
class XFileUtil {
  ...
  static void sendXToBroker(ArrTable<DoubleArray, DoubleArrPlus> table, int d,
		    boolean isMaster, String labelFile, GoogleActiveMQSender sender, boolean isLabeled) throws IOException {
		    if (isMaster) {
		      ...
		      for (ArrPartition<DoubleArray> partition : partitions) {
		        ...
		        for (int i = 0; i < parArray.getSize(); i++) {
		        	if (i % (d) == 0) {
		        		...
		        		z = doubles[i];
		        		sender.setPosition(label, x, y, z);
		        	} 
		          
		        }
		      }
		   // send activemq message
		      System.out.print("Send Message to Broker!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
		      sender.sendMessage();
		    }
		  }
}
