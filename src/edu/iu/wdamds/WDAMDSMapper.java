...
import cgl.pviz.rpc.GoogleActiveMQSender;
...
public class WDAMDSMapper extends
  ...
  public void runMDS(String partitionFile, Context context) throws Exception {
    ...
    while (tCur > tMin) {
     ...
     // Send X file to Broker
        LOG.info("Send X (data points) to Broker");
        if (!labelsFile.endsWith("NoLabel") && iter == 0) {
          XFileUtil.sendXToBroker(xTable, d, this.isMaster(), labelsFile, sender, true);
        } else {
          XFileUtil.sendXToBroker(xTable, d, this.isMaster(), labelsFile, sender, false);
        }
